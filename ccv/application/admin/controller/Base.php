...
class Base extends Controller
{
	...
	public function _initialize() {
        ...
        $user = Db::name('administrator')->alias('a')->join('so_auth_access b','a.admin_id = b.uid')->join('so_auth_group c','b.group_id = c.id')->where('a.admin_id',$userid)->where('a.state',0)->field('a.*,c.title,c.id')->find();
        if(empty($user)){
